...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_skp'])) {
    ...
    if (isset($_POST['utama_id'])) {
        foreach ($_POST['utama_id'] as $i => $id) {
            if ($is_skp_akhir) {
                // For SKP Akhir, use skp_akhir_pegawai table
                $stmt = $conn->prepare("UPDATE skp_akhir_pegawai SET RHK_PIMPINAN_INTERV=?, RENCANA_HASIL_KERJA=?, ASPEK=?, INDIKATOR_KINERJA_INDIVIDU=?, TARGET=?, REALISASI_BERDASARKAN_BUKTI_DUKUNG=?, SATUAN=? WHERE ID_SKP=? AND NIP=?");
            } else {
                // For regular SKP, use skp_pegawai table
                $stmt = $conn->prepare("UPDATE skp_pegawai SET RHK_PIMPINAN_INTERV=?, RENCANA_HASIL_KERJA=?, ASPEK=?, INDIKATOR_KINERJA_INDIVIDU=?, TARGET=?, REALISASI_BERDASARKAN_BUKTI_DUKUNG=?, SATUAN=? WHERE id=? AND NIP=?");
            }
