...
if (!$is_atasan) {
    ...
    $user_nip = $_SESSION['nip'] ?? '';
    $count_sql = "SELECT COUNT(DISTINCT id_skp_global) as total FROM skp_pegawai WHERE NIP = ?";
    ...
    if ($view_employee_skp) {
        ...
        $count_where_conditions = ["NAMA_ATASAN_LANGSUNG = ?", "(STATUS = 'SUBMITTED' OR STATUS = 'PROSES EVALUASI' OR STATUS = 'SELESAI EVALUASI')"];
        $count_params = [$manager_name];
        $count_param_types = 's';
        
        if (!empty($filter_nama)) {
            $count_where_conditions[] = "NAMA LIKE ?";
            $count_params[] = '%' . $filter_nama . '%';
            $count_param_types .= 's';
        }
        
        if (!empty($filter_tahun)) {
            $count_where_conditions[] = "TAHUN = ?";
            $count_params[] = $filter_tahun;
            $count_param_types .= 'i';
        }
        
        $count_sql = "SELECT COUNT(DISTINCT id_skp_global) as total FROM skp_pegawai WHERE " . implode(' AND ', $count_where_conditions);
        $count_stmt = $conn->prepare($count_sql);
...
if (!$is_atasan) {
    ...
    if ($view_employee_skp) {
        ...
        $main_where_conditions = ["s.NAMA_ATASAN_LANGSUNG = ?", "(s.STATUS = 'SUBMITTED' OR s.STATUS = 'PROSES EVALUASI' OR s.STATUS = 'SELESAI EVALUASI')"];
        $main_params = [$manager_name];
        $main_param_types = 's';
...
<body>
                                        ...
                                        <table class="data-table">
                                            ...
                                            <tbody>
                                                ...
                                                foreach ($year_group['records'] as $row): ?>
                                                    <tr>
                                                        <td><?= $counter++ ?></td>
                                                        <td><strong>Triwulan <?= htmlspecialchars($row['TRIWULAN'] ?? 'N/A') ?></strong></td>
                                                        ...
                                                        <td>
                                                            <div class="aksi-buttons">
                                                                ...
                                                                <?php if ($view_employee_skp && $is_atasan): ?>
                                                                    <!-- Manager viewing employee SKP for evaluation -->
                                                                    <?php if (($row['STATUS'] ?? '') === 'SELESAI EVALUASI'): ?>
                                                                        <button class="download-pdf-btn" onclick="downloadPDF(<?= htmlspecialchars($row['id_skp_global']) ?>, '<?= htmlspecialchars($row['ESELON_PEGAWAI'] ?? '') ?>')"><img src="images/pdf.png" style="height:16px;vertical-align:middle;margin-right:4px;" alt="PDF"> Evaluasi Kuantitatif</button>
                                                                        <button class="download-pdf-btn" onclick="downloadKuantitatifPDF(<?= htmlspecialchars($row['id_skp_global']) ?>, '<?= htmlspecialchars($row['ESELON_PEGAWAI'] ?? '') ?>')" style="background: linear-gradient(135deg, #fd7e14 0%, #e8590c 100%); box-shadow: 0 2px 8px rgba(253, 126, 20, 0.3);"><img src="images/pdf.png" style="height:16px;vertical-align:middle;margin-right:4px;" alt="PDF"> Kuantitatif</button>
                                                                        <button class="download-pdf-btn" onclick="downloadUmpanBalikPDF(<?= htmlspecialchars($row['id_skp_global']) ?>, '<?= htmlspecialchars($row['ESELON_PEGAWAI'] ?? '') ?>')" style="background: linear-gradient(135deg, #20c997 0%, #17a2b8 100%); box-shadow: 0 2px 8px rgba(32, 201, 151, 0.3);"><img src="images/pdf.png" style="height:16px;vertical-align:middle;margin-right:4px;" alt="PDF"> Umpan Balik Kuantitatif</button>
                                                                        <button class="download-pdf-btn" onclick="downloadDokumenEvaluasiTriwulanPDF(<?= htmlspecialchars($row['id_skp_global']) ?>, '<?= htmlspecialchars($row['ESELON_PEGAWAI'] ?? '') ?>')" style="background: linear-gradient(135deg, #6f42c1 0%, #5a32a3 100%); box-shadow: 0 2px 8px rgba(111, 66, 193, 0.3);"><img src="images/pdf.png" style="height:16px;vertical-align:middle;margin-right:4px;" alt="PDF"> Dokumen Evaluasi</button>
                                                                        <button class="submit-evaluasi-btn" disabled style="background: #6c757d; cursor: not-allowed; opacity: 0.6;">Selesai Evaluasi</button>
                                                                    <?php elseif (($row['STATUS'] ?? '') === 'PROSES EVALUASI'): ?>
                                                                        <?php 
                                                                        $eval_page = (isset($row['ESELON_PEGAWAI']) && $row['ESELON_PEGAWAI'] === 'YA') ? 'skp_detail_evaluasi_eselon.php' : 'skp_detail.php';
                                                                        ?>
                                                                        <button class="edit-btn view-details-btn" style="background: linear-gradient(135deg, #ffc107 0%, #ff9800 100%); box-shadow: 0 2px 8px rgba(255, 193, 7, 0.3); color: white;" onclick="window.location.href='<?= $eval_page ?>?id_skp_global=<?= htmlspecialchars($row['id_skp_global']) ?>&edit_feedback=1'">EVALUASI</button>
                                                                        <button class="revisi-skp-btn" onclick="revisiSKP(<?= htmlspecialchars($row['id_skp_global']) ?>)">REVISI SKP</button>
                                                                        <button class="submit-evaluasi-btn" onclick="submitEvaluasi(<?= htmlspecialchars($row['id_skp_global']) ?>)">Submit Evaluasi</button>
                                                                    <?php elseif (($row['STATUS'] ?? '') === 'SUBMITTED'): ?>
                                                                        <?php 
                                                                        $eval_page = (isset($row['ESELON_PEGAWAI']) && $row['ESELON_PEGAWAI'] === 'YA') ? 'skp_detail_evaluasi_eselon.php' : 'skp_detail.php';
                                                                        ?>
                                                                        <button class="edit-btn view-details-btn" style="background: linear-gradient(135deg, #17a2b8 0%, #138496 100%); box-shadow: 0 2px 8px rgba(23, 162, 184, 0.3); color: white;" onclick="window.location.href='<?= $eval_page ?>?id_skp_global=<?= htmlspecialchars($row['id_skp_global']) ?>&edit_feedback=1'">EVALUASI</button>
                                                                    ...
                                                                    <?php endif; ?>
                                                                <?php else: ?>
                                                                    <!-- Employee viewing their own SKP -->
                                                                    <?php if (strtoupper($row['STATUS'] ?? '') === 'DRAFT' || strtoupper($row['STATUS'] ?? '') === 'DRAFT DIKEMBALIKAN'): ?>
                                                                        <button class="edit-btn view-details-btn" style="background: linear-gradient(135deg, #ffc107 0%, #ff9800 100%); box-shadow: 0 2px 8px rgba(255, 193, 7, 0.3); color: white;" onclick="window.location.href='<?= $is_eselon ? 'skpbaru_eselon.php' : 'skpbaru.php' ?>?id_skp_global=<?= htmlspecialchars($row['id_skp_global']) ?>'">UBAH</button>
                                                                    ...
                                                                    <?php elseif (strtoupper($row['STATUS'] ?? '') === 'SUBMITTED'): ?>
                                                                        <button class="edit-btn view-details-btn" style="background: linear-gradient(135deg, #17a2b8 0%, #138496 100%); box-shadow: 0 2px 8px rgba(23, 162, 184, 0.3); color: white;" onclick="editRealisasi(<?= htmlspecialchars($row['id_skp_global']) ?>)">UBAH REALISASI</button>
                                                                        <button class="submit-evaluasi-btn" onclick="submitSKPFinal(<?= htmlspecialchars($row['id_skp_global']) ?>)">Kirim SKP</button>
                                                                    <?php elseif (strtoupper($row['STATUS'] ?? '') === 'PROSES EVALUASI'): ?>
                                                                        <button class="submit-evaluasi-btn" disabled style="background: #6c757d; cursor: not-allowed; opacity: 0.6;">⏳ Sedang Dievaluasi</button>
                                                                    <?php elseif (strtoupper($row['STATUS'] ?? '') === 'SELESAI EVALUASI'): ?>
                                                                        <button class="download-pdf-btn" onclick="downloadPDF(<?= htmlspecialchars($row['id_skp_global']) ?>, '<?= htmlspecialchars($row['ESELON_PEGAWAI'] ?? '') ?>')"><img src="images/pdf.png" style="height:16px;vertical-align:middle;margin-right:4px;" alt="PDF"> Evaluasi Kuantitatif</button>
                                                                        <button class="download-pdf-btn" onclick="downloadKuantitatifPDF(<?= htmlspecialchars($row['id_skp_global']) ?>, '<?= htmlspecialchars($row['ESELON_PEGAWAI'] ?? '') ?>')" style="background: linear-gradient(135deg, #fd7e14 0%, #e8590c 100%); box-shadow: 0 2px 8px rgba(253, 126, 20, 0.3);"><img src="images/pdf.png" style="height:16px;vertical-align:middle;margin-right:4px;" alt="PDF"> Kuantitatif</button>
                                                                        <button class="download-pdf-btn" onclick="downloadUmpanBalikPDF(<?= htmlspecialchars($row['id_skp_global']) ?>, '<?= htmlspecialchars($row['ESELON_PEGAWAI'] ?? '') ?>')" style="background: linear-gradient(135deg, #20c997 0%, #17a2b8 100%); box-shadow: 0 2px 8px rgba(32, 201, 151, 0.3);"><img src="images/pdf.png" style="height:16px;vertical-align:middle;margin-right:4px;" alt="PDF"> Umpan Balik Kuantitatif</button>
                                                                        <button class="download-pdf-btn" onclick="downloadDokumenEvaluasiTriwulanPDF(<?= htmlspecialchars($row['id_skp_global']) ?>, '<?= htmlspecialchars($row['ESELON_PEGAWAI'] ?? '') ?>')"><img src="images/pdf.png" style="height:16px;vertical-align:middle;margin-right:4px;" alt="PDF"> Dokumen Evaluasi</button>
    ...
    <script>
        function viewSKPDetails(idSkpGlobal, eselonPegawai) {
            // Open SKP details in a new window/tab
            var detailPage = (eselonPegawai === 'YA') ? "skp_detail_eselon.php" : "skp_detail.php";
        ...
        function downloadPDF(idSkpGlobal, eselon) {
            ...
            const path = (eselon === 'YA') ? 'skp_eselon/generate_pdf.php' : 'generate_pdf.php';
        ...
        function downloadUmpanBalikPDF(idSkpGlobal, eselon) {
            ...
            const path = (eselon === 'YA') ? 'skp_eselon/generate_umpan_balik_quarterly_pdf.php' : 'generate_umpan_balik_quarterly_pdf.php';
            window.open(path + '?id_skp_global=' + idSkpGlobal, '_blank');
        ...
        function downloadDokumenEvaluasiTriwulanPDF(idSkpGlobal, eselon) {
            ...
            const path = (eselon === 'YA') ? 'skp_eselon/generate_dokumen_evaluasi_triwulan_pdf.php' : 'generate_dokumen_evaluasi_triwulan_pdf.php';
            window.open(path + '?id_skp_global=' + idSkpGlobal, '_blank');
