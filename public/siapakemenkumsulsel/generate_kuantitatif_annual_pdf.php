...
// Fetch SKP data - Annual SKP only (from skp_akhir_pegawai)
$is_annual = true;
$skp_data = [];
...
// Fetch Pegawai (dinilai) detail from historical Q1 data
$pegawai_detail = null;
if (!empty($skp_data[0]['NIP']) && !empty($skp_data[0]['TAHUN'])) {
    $nip_pegawai = $skp_data[0]['NIP'];
    $tahun = $skp_data[0]['TAHUN'];
    $q1_sql = "SELECT NAMA, NIP, PANGKAT_GOL_RUANG, JABATAN, UNIT_KERJA, SATUAN_KERJA FROM skp_pegawai WHERE NIP = ? AND TAHUN = ? AND TRIWULAN = 1 LIMIT 1";
...
// Set period display text - Annual only
$periode_display = 'AKHIR';
...
<head>
    <meta charset="UTF-8">
    <title>SKP Kuantitatif - <?= htmlspecialchars($employee_name) ?> - <?= htmlspecialchars($period) ?> - <?= htmlspecialchars($tahun) ?></title>
...
<body>
    ...
    <div class="header-title">PERIODE: <?= htmlspecialchars($periode_display) ?></div>
    <br>
