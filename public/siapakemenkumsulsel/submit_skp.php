...
try {
    ...
    // Update status in skp_pegawai table to 'SUBMITTED' (only for DRAFT or DRAFT DIKEMBALIKAN)
    $stmt1 = $conn->prepare("UPDATE skp_pegawai SET STATUS = 'SUBMITTED' WHERE id_skp_global = ? AND NIP = ? AND (STATUS = 'DRAFT' OR STATUS = 'DRAFT DIKEMBALIKAN')");
    if (!$stmt1) {
    ...
    // Update status in skp_perilaku_pegawai table to 'SUBMITTED' (only for DRAFT or DRAFT DIKEMBALIKAN)
    $stmt2 = $conn->prepare("UPDATE skp_perilaku_pegawai SET STATUS = 'SUBMITTED' WHERE id_skp_global = ? AND NIP = ? AND (STATUS = 'DRAFT' OR STATUS = 'DRAFT DIKEMBALIKAN')");
    if (!$stmt2) {
