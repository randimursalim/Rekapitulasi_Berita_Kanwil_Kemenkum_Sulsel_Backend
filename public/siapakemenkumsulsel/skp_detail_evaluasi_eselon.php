...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['evaluate_skp'])) {
    ...
    if (isset($_POST['skp_feedback']) && is_array($_POST['skp_feedback'])) {
        ...
        foreach ($skp_feedback as $index => $feedback) {
            try {
                ...
                if ($nip) {
                    $stmt = $conn->prepare("UPDATE skp_pegawai SET UMPAN_BALIK_DENGAN_BUKTI_DUKUNG = ?, UMPAN_BALIK_STICKER = ?, TANGGAL_EVALUASI_SKP = NOW() WHERE id_skp_global = ? AND NIP = ?");
                    if ($stmt) {
    ...
    if (isset($_POST['rating_perilaku_kerja']) || isset($_POST['predikat_kinerja_pegawai']) || isset($_POST['rating_hasil_kerja']) || isset($_POST['capaian_kinerja_organisasi']) || isset($_POST['umpan_balik_dengan_bukti_dukung']) || isset($_POST['umpan_balik_sticker'])) {
        ...
        if (!empty($rating_fields)) {
            ...
            if ($is_skp_akhir) {
                ...
                $sql = "UPDATE skp_akhir_pegawai SET " . implode(', ', $rating_fields) . ", TANGGAL_EVALUASI_SKP = NOW() WHERE ID_SKP_GLOBAL = ?";
            } else {
                // For regular SKP, use skp_pegawai table
                $sql = "UPDATE skp_pegawai SET " . implode(', ', $rating_fields) . ", TANGGAL_EVALUASI_SKP = NOW() WHERE id_skp_global = ?";
            }
    ...
    if ($update_success) {
        // For SKP Akhir, update status to SELESAI EVALUASI
        if ($is_skp_akhir) {
            // Update skp_akhir_pegawai status
            $status_update_sql = "UPDATE skp_akhir_pegawai SET STATUS = 'SELESAI EVALUASI', TANGGAL_EVALUASI_SKP = NOW() WHERE ID_SKP_GLOBAL = ? AND STATUS = 'PROSES EVALUASI'";
            $status_stmt = $conn->prepare($status_update_sql);
            if ($status_stmt) {
                $status_stmt->bind_param('i', $id_skp_global);
                $status_stmt->execute();
                $status_stmt->close();
            }
            
            // Update skp_akhir_perilaku_pegawai status
            $perilaku_status_sql = "UPDATE skp_akhir_perilaku_pegawai SET STATUS = 'SELESAI EVALUASI' WHERE ID_SKP_GLOBAL = ? AND STATUS = 'PROSES EVALUASI'";
            $perilaku_status_stmt = $conn->prepare($perilaku_status_sql);
            if ($perilaku_status_stmt) {
                $perilaku_status_stmt->bind_param('i', $id_skp_global);
                $perilaku_status_stmt->execute();
                $perilaku_status_stmt->close();
            }
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_skp'])) {
    ...
    if (isset($_POST['utama_id'])) {
        foreach ($_POST['utama_id'] as $i => $id) {
            if ($is_skp_akhir) {
                // For SKP Akhir, use skp_akhir_pegawai table
                $stmt = $conn->prepare("UPDATE skp_akhir_pegawai SET RHK_PIMPINAN_INTERV=?, RENCANA_HASIL_KERJA=?, ASPEK=?, INDIKATOR_KINERJA_INDIVIDU=?, TARGET=?, REALISASI_BERDASARKAN_BUKTI_DUKUNG=?, SATUAN=?, PERSPEKTIF=? WHERE ID_SKP=? AND NIP=?");
            } else {
                // For regular SKP, use skp_kuantitatif_awal_tahun_pegawai table
                $stmt = $conn->prepare("UPDATE skp_kuantitatif_awal_tahun_pegawai SET RHK_PIMPINAN_INTERV=?, RENCANA_HASIL_KERJA=?, ASPEK=?, INDIKATOR_KINERJA_INDIVIDU=?, TARGET=?, REALISASI_BERDASARKAN_BUKTI_DUKUNG=?, SATUAN=?, PERSPEKTIF=? WHERE id=? AND NIP=?");
            }
            $rhk_pimpinan = $_POST['utama_pimpinan'][$i] ?? '';
            $aspek = $_POST['utama_aspek'][$i] ?? '';
    ...
    if (isset($_POST['tambahan_id'])) {
        foreach ($_POST['tambahan_id'] as $i => $id) {
            if ($is_skp_akhir) {
                ...
                // For regular SKP, use skp_pegawai table
                $stmt = $conn->prepare("UPDATE skp_pegawai SET RHK_PIMPINAN_INTERV=?, RENCANA_HASIL_KERJA=?, ASPEK=?, INDIKATOR_KINERJA_INDIVIDU=?, TARGET=?, REALISASI_BERDASARKAN_BUKTI_DUKUNG=?, SATUAN=?, PERSPEKTIF=? WHERE id=? AND NIP=?");
            }
            $rhk_pimpinan = $_POST['tambahan_pimpinan'][$i] ?? '';
            $aspek = $_POST['tambahan_aspek'][$i] ?? '';
    ...
    header('Location: skp_detail_evaluasi_eselon.php?id_skp_global=' . $id_skp_global);
    exit();
}
...
<body>
    <div class="container">
        ...
        <div class="content">
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <?php if ($is_draft): ?>
                                            ...
                                            <td><textarea name="utama_perspektif[<?= $index ?>]" class="evaluation-textarea" required><?= htmlspecialchars($row['PERSPEKTIF'] ?? '') ?></textarea></td>
                                        ...
                                        <?php elseif ($is_edit_feedback): ?>
                                            ...
                                            <td><div class="skp-field-content" style="background: #ffffcc; border: 2px solid #ff0000; font-weight: bold;">SATUAN: <?= htmlspecialchars($row['SATUAN'] ?? 'N/A') ?></div></td>
                                            <td><div class="skp-field-content"><?= nl2br(htmlspecialchars($row['PERSPEKTIF'] ?? 'N/A')) ?></div></td>
                                        <?php else: ?>
                                            <td><div class="skp-field-content"><?= nl2br(htmlspecialchars($row['RENCANA_HASIL_KERJA'])) ?></div></td>
                                            <td><div class="skp-field-content"><?= nl2br(htmlspecialchars($row['INDIKATOR_KINERJA_INDIVIDU'])) ?></div></td>
                                            <td><div class="skp-field-content"><?= nl2br(htmlspecialchars($row['TARGET'])) ?></div></td>
                            ...
                            <?php elseif ($is_edit_feedback): ?>
                                <div style="display: flex; align-items: flex-start; gap: 10px; margin-bottom: 10px;">
                                    <div style="display: flex; flex-direction: column; align-items: center; min-width: 50px;">
                                        <label style="display: flex; align-items: center; cursor: pointer; font-size: 30px; color: #0D2052; line-height: 1;">
                                            <input type="checkbox" name="skp_stickers[<?= $index ?>]" value="1" <?= (!empty($row['UMPAN_BALIK_STICKER']) && $row['UMPAN_BALIK_STICKER'] == 'C') ? 'checked' : '' ?> style="margin-right: 5px; transform: scale(1.1); width: 14px; height: 14px;" class="skp-sticker-checkbox" data-index="<?= $index ?>">
                                            👍 
                                        </label>
                                        <small style="font-size: 9px; color: #6c757d; text-align: center; margin-top: 2px;"></small>
                                    </div>
                                    <div style="flex: 1;">
                                        <label style="font-size: 12px; color: #856404; font-weight: bold; margin-bottom: 5px; display: block;">* </label>
                                        <textarea name="skp_feedback[<?= $index ?>]" class="evaluation-textarea skp-feedback-textarea" placeholder="Masukkan umpan balik untuk kinerja utama..." id="skp_feedback_<?= $index ?>" style="width: 100%;" data-index="<?= $index ?>"><?= htmlspecialchars($row['UMPAN_BALIK_DENGAN_BUKTI_DUKUNG'] ?? '') ?></textarea>
                                    </div>
                                </div>
                            <?php else: ?>
                                <div style="display: flex; align-items: flex-start; gap: 10px;">
                                    <div style="display: flex; flex-direction: column; align-items: center; min-width: 30px;">
                                        <?php if (!empty($row['UMPAN_BALIK_STICKER']) && $row['UMPAN_BALIK_STICKER'] == 'C'): ?>
                                            <span style="font-family: 'Wingdings', 'Wingdings 2', 'Wingdings 3', 'Arial', sans-serif; font-size: 30px; color: #28a745;">C</span>
                                        <?php endif; ?>
                                    </div>
                                    <div style="flex: 1;">
                                        <div class="evaluation-textarea" style="background: #f8f9fa; border: 1px solid #0D2052; border-radius: 6px; padding: 10px; font-size: 14px; min-height: 100px; white-space: pre-line;" readonly>
                    ...
                    <div class="skp-item-with-feedback" style="position: relative;">
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <?php if ($is_draft): ?>
                                            ...
                                            <td><textarea name="tambahan_perspektif[<?= $index ?>]" class="evaluation-textarea"><?= htmlspecialchars($row['PERSPEKTIF'] ?? '') ?></textarea></td>
                        ...
                        <div class="feedback-form-sidebar">
                            ...
                            <?php elseif ($is_edit_feedback): ?>
                                <div style="display: flex; align-items: flex-start; gap: 10px; margin-bottom: 10px;">
                                    <div style="display: flex; flex-direction: column; align-items: center; min-width: 50px;">
                                        <label style="display: flex; align-items: center; cursor: pointer; font-size: 18px; color: #0D2052; line-height: 1;">
                                            <input type="checkbox" name="skp_stickers[<?= $index ?>]" value="1" <?= (!empty($row['UMPAN_BALIK_STICKER']) && $row['UMPAN_BALIK_STICKER'] == 'C') ? 'checked' : '' ?> style="margin-right: 5px; transform: scale(1.1); width: 14px; height: 14px;" class="skp-sticker-checkbox" data-index="<?= $index ?>">
                                            👍 
                                        </label>
                                        <small style="font-size: 9px; color: #6c757d; text-align: center; margin-top: 2px;">(Opsional)</small>
                                    </div>
                                    <div style="flex: 1;">
                                        <label style="font-size: 12px; color: #856404; font-weight: bold; margin-bottom: 5px; display: block;">* Pilih sticker atau tulis feedback (minimal salah satu)</label>
                                        <textarea name="skp_feedback[<?= $index ?>]" class="evaluation-textarea skp-feedback-textarea" placeholder="Masukkan umpan balik untuk kinerja tambahan..." id="skp_feedback_tambahan_<?= $index ?>" style="width: 100%;" data-index="<?= $index ?>"><?= htmlspecialchars($row['UMPAN_BALIK_DENGAN_BUKTI_DUKUNG'] ?? '') ?></textarea>
                                    </div>
                                </div>
                            <?php else: ?>
                                <div style="display: flex; align-items: flex-start; gap: 10px;">
            ...
            ?>
                <div class="skp-item-with-feedback">
                    ...
                    <div class="feedback-form-sidebar">
                        ...
                        <?php if ($is_edit_feedback): ?>
                            <label style="font-size: 12px; color: #dc3545; font-weight: bold; margin-bottom: 5px; display: block;">* Wajib diisi</label>
                            <textarea name="perilaku_feedback[<?php echo $key; ?>]" class="evaluation-textarea" placeholder="Masukkan umpan balik untuk perilaku kerja <?php echo $title; ?>..." id="perilaku_feedback_<?php echo $key; ?>" required><?php echo htmlspecialchars($current_feedback); ?></textarea>
                        <?php else: ?>
                            <div class="evaluation-textarea" style="background: #f8f9fa; border: 1px solid #0D2052; border-radius: 6px; padding: 10px; font-size: 14px; min-height: 100px; white-space: pre-line;" readonly><?php echo htmlspecialchars($current_feedback ?: 'Belum ada umpan balik'); ?></div>
                        <?php endif; ?>
                    </div>
                </div>
            <?php 
            ...
            <!-- Rating and Predikat Display Section for All Users -->
            <?php if (!empty($first_row['RATING_PERILAKU_KERJA']) || !empty($first_row['PREDIKAT_KINERJA_PEGAWAI'])): ?>
            ...
            <?php if ($is_edit_feedback): ?>
                <div style="text-align: center; margin: 30px 0; padding: 20px; background: #f8f9fa; border-radius: 8px; border: 1px solid #0D2052;">
                    <?php if ($is_skp_akhir): ?>
                        <button type="submit" style="background: #28a745; color: white; border: none; padding: 12px 30px; border-radius: 6px; font-size: 16px; font-weight: bold; cursor: pointer; transition: background 0.3s;">✅ SUBMIT EVALUASI SKP AKHIR</button>
                        <button type="button" onclick="window.location.href='skp_akhir_evaluasi.php'" style="background: #6c757d; color: white; border: none; padding: 12px 30px; border-radius: 6px; font-size: 16px; font-weight: bold; cursor: pointer; margin-left: 10px;">❌ BATAL</button>
                    <?php else: ?>
                        <button type="submit" style="background: #0D2052; color: white; border: none; padding: 12px 30px; border-radius: 6px; font-size: 16px; font-weight: bold; cursor: pointer; transition: background 0.3s;">💾 SIMPAN UMPAN BALIK</button>
            ...
            <!-- Edit Realisasi Button for SUBMITTED Status -->
            <?php if ($is_submitted && $is_owner && !$is_skp_akhir): ?>
                <div style="text-align: center; margin: 30px 0; padding: 20px; background: #e3f2fd; border-radius: 8px; border: 1px solid #2196f3;">
                    <p style="margin-bottom: 15px; color: #1976d2; font-size: 14px;">
                        <strong>Status: SUBMITTED</strong> - Hanya kolom Realisasi yang dapat diedit (SATUAN read-only)
                    </p>
                    <button type="button" onclick="window.open('edit_realisasi.php?id_skp_global=<?= htmlspecialchars($id_skp_global) ?>', '_blank', 'width=1200,height=800,scrollbars=yes,resizable=yes')" style="background: linear-gradient(135deg, #17a2b8 0%, #138496 100%); color: white; border: none; padding: 12px 30px; border-radius: 6px; font-size: 16px; font-weight: bold; cursor: pointer; box-shadow: 0 2px 8px rgba(23, 162, 184, 0.3);">
                        📝 UBAH REALISASI
                    </button>
                </div>
            <?php endif; ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (form) {
        form.addEventListener('submit', function(e) {
            ...
            // Check SKP feedback (Kinerja Utama & Tambahan) - either sticker OR text is required
            const skpTextareas = form.querySelectorAll('.skp-feedback-textarea');
            skpTextareas.forEach(function(textarea) {
                const index = textarea.getAttribute('data-index');
                const checkbox = form.querySelector('.skp-sticker-checkbox[data-index="' + index + '"]');
                const hasText = textarea.value.trim() !== '';
                const hasSticker = checkbox && checkbox.checked;
                
                if (!hasText && !hasSticker) {
                    isValid = false;
                    emptyFields.push('Umpan Balik Kinerja (index ' + (parseInt(index) + 1) + ') - harus mengisi text atau memilih sticker');
                }
            });
            
            // Check Perilaku Kerja feedback - text is required
            const perilakuTextareas = form.querySelectorAll('textarea[name^="perilaku_feedback"]');
            perilakuTextareas.forEach(function(textarea) {
                if (!textarea.value.trim()) {
                    isValid = false;
                    const fieldName = textarea.getAttribute('id').replace('perilaku_feedback_', '');
                    emptyFields.push('Umpan Balik Perilaku Kerja: ' + fieldName);
                }
            });
            
            if (!isValid) {
                e.preventDefault();
                alert('Mohon lengkapi semua umpan balik yang wajib diisi:\n\n' + emptyFields.join('\n'));
                return false;
            }
    ...
    // Add visual feedback for SKP feedback fields (either sticker or text)
    const skpTextareas = document.querySelectorAll('.skp-feedback-textarea');
    skpTextareas.forEach(function(textarea) {
        const index = textarea.getAttribute('data-index');
        const checkbox = document.querySelector('.skp-sticker-checkbox[data-index="' + index + '"]');
    ...
    // Add visual feedback for Perilaku Kerja fields (text required)
    const perilakuTextareas = document.querySelectorAll('textarea[name^="perilaku_feedback"]');
    perilakuTextareas.forEach(function(textarea) {
