...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif (isset($_SESSION['last_submit_time']) && (time() - $_SESSION['last_submit_time']) < 5) {
        ...
        if (empty($errorMsg)) {
            ...
            $utama_realisasi = $_POST['utama_realisasi'] ?? [];
            $count = max(count($utama_pimpinan), count($utama_kerja), count($utama_aspek), count($utama_indikator), count($utama_target), count($utama_realisasi));
            ...
            for ($i = 0; $i < $count; $i++) {
                ...
                $r6 = trim($utama_realisasi[$i] ?? '');
                $any = ($r1 !== '' || $r2 !== '' || $r3 !== '' || $r4 !== '' || $r5 !== '' || $r6 !== '');
        ...
        foreach ($triwulan_list as $triwulan) {
            ...
            if (empty($errorMsg) && isset($_POST['utama_pimpinan']) && !empty(array_filter($_POST['utama_pimpinan']))) {
                ...
                for ($i = 0; $i < $count; $i++) {
                    ...
                    if ($rhk !== '' && $rencana !== '' && $aspek !== '' && $indikator !== '' && $target !== '' && $realisasi !== '' && $satuan !== '') {
                        ...
                        if ($stmt) {
                            ...
                            if ($triwulan == 1 && !$edit_mode) {
                                ...
                                if ($stmt_k) {
                                    $stmt_k->bind_param('ssssssssssssssssssis', $namaPegawai, $nipPegawai, $namaAtasan, $nipAtasan, $pangkatPegawai, $jabatanPegawai, $unitKerjaPegawai, $satuanKerjaPegawai, $rhk, $rencana, $aspek, $indikator, $target, $realisasi, $satuan, $triwulan, $tahun, $status, $id_kuantitatif_global, $jenisKinerjaUtama);
                                    $stmt_k->execute();
                                    $stmt_k->close();
                                }
                            }
            ...
            if (isset($_POST['tambahan_pimpinan']) && !empty(array_filter($_POST['tambahan_pimpinan']))) {
                ...
                for ($i = 0; $i < $count; $i++) {
                    ...
                    // Only insert if there's actual content (ignoring aspek which has a default value)
                    if (trim($rhk) !== '' || trim($rencana) !== '' || trim($indikator) !== '' || trim($target) !== '' || trim($realisasi) !== '' || trim($satuan) !== '') {
                        $stmt = $conn->prepare("INSERT INTO skp_pegawai (NAMA, NIP, NAMA_ATASAN_LANGSUNG, NIP_ATASAN_LANGSUNG, PANGKAT_GOL_RUANG, JABATAN, UNIT_KERJA, SATUAN_KERJA, RHK_PIMPINAN_INTERV, RENCANA_HASIL_KERJA, ASPEK, INDIKATOR_KINERJA_INDIVIDU, TARGET, REALISASI_BERDASARKAN_BUKTI_DUKUNG, SATUAN, TRIWULAN, TAHUN, STATUS, TANGGAL_INPUT_SKP, ID_SKP_GLOBAL, JENIS_KINERJA) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW() + INTERVAL 8 HOUR, ?, ?)");
                        if ($stmt) {
                            ...
                            if ($triwulan == 1 && !$edit_mode) {
                                $stmt_k = $conn->prepare("INSERT INTO skp_kuantitatif_awal_tahun_pegawai (NAMA, NIP, NAMA_ATASAN_LANGSUNG, NIP_ATASAN_LANGSUNG, PANGKAT_GOL_RUANG, JABATAN, UNIT_KERJA, SATUAN_KERJA, RHK_PIMPINAN_INTERV, RENCANA_HASIL_KERJA, ASPEK, INDIKATOR_KINERJA_INDIVIDU, TARGET, REALISASI_BERDASARKAN_BUKTI_DUKUNG, SATUAN, TRIWULAN, TAHUN, STATUS, TANGGAL_INPUT_SKP, ID_SKP_GLOBAL, JENIS_KINERJA) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW() + INTERVAL 8 HOUR, ?, ?)");
                                if ($stmt_k) {
                                    $stmt_k->bind_param('ssssssssssssssssssis', $namaPegawai, $nipPegawai, $namaAtasan, $nipAtasan, $pangkatPegawai, $jabatanPegawai, $unitKerjaPegawai, $satuanKerjaPegawai, $rhk, $rencana, $aspek, $indikator, $target, $realisasi, $satuan, $triwulan, $tahun, $status, $id_kuantitatif_global, $jenisKinerjaTambahan);
                                    $stmt_k->execute();
                                    $stmt_k->close();
                                }
                            }
...
<head>
    ...
    <title><?= $edit_mode ? 'Edit SKP' : 'SKP Baru' ?> - SI-APA</title>
    ...
    <style>
        * {
        ...
        .skp-grid-header {
            ...
            border-radius: 6px;
            display: grid;
            grid-template-columns: 40px 1.5fr 1.5fr 0.9fr 1.2fr 0.9fr 1.2fr 0.8fr 56px;
            gap: 8px;
            font-weight: 600;
            margin-bottom: 6px;
            font-size: 0.75rem;
        ...
        @media (min-width: 1200px) {
            .skp-grid-header {
                grid-template-columns: 44px 1.5fr 1.5fr 0.9fr 1.2fr 0.9fr 1.2fr 0.8fr 60px;
                gap: 10px;
                padding: 10px 12px;
                font-size: 0.8rem;
            }
        ...
        @media (min-width: 1600px) {
            .skp-grid-header {
                grid-template-columns: 48px 1.5fr 1.5fr 0.9fr 1.2fr 0.9fr 1.2fr 0.8fr 60px;
                gap: 10px;
                padding: 10px 12px;
            }
            .skp-grid {
            ...
            <div class="skp-grid" id="utama-table">
                <?php if ($edit_mode && !empty($edit_utama_data)): ?>
                    <?php foreach ($edit_utama_data as $index => $item): ?>
                        <div class="skp-item">
                            ...
                            <div class="skp-cell" data-label="Realisasi"><textarea class="skp-textarea" name="utama_realisasi[]" required><?= htmlspecialchars($item['REALISASI_BERDASARKAN_BUKTI_DUKUNG'] ?? '') ?></textarea></div>
                ...
                <?php else: ?>
                    <div class="skp-item">
                        ...
                        <div class="skp-cell" data-label="Realisasi"><textarea class="skp-textarea" name="utama_realisasi[]" required></textarea></div>
...
- Menggerakan pemanfaatan berbagai sumber daya untuk tujuan bersama.</div>
    ...
    function addRow(tableId) {
        ...
        wrapper.innerHTML =
            ...
            '<div class="skp-cell" data-label="Realisasi"><textarea class="skp-textarea" ' + (isUtama ? 'required ' : '') + 'name="' + (isUtama ? 'utama_realisasi[]' : 'tambahan_realisasi[]') + '"></textarea></div>' +
