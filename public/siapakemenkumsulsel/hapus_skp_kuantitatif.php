...
try {
    $stmt1 = $conn->prepare('DELETE FROM skp_kuantitatif_awal_tahun_pegawai WHERE id_skp_global = ?');
    $stmt1->bind_param('i', $id_skp_global);
    $stmt1->execute();
    $stmt2 = $conn->prepare('DELETE FROM skp_perilaku_awal_tahun_pegawai WHERE id_skp_global = ?');
