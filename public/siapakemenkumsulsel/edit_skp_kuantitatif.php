...
if ($edit_id_skp_global) {
    ...
    $sql_utama = "SELECT * FROM skp_kuantitatif_awal_tahun_pegawai WHERE id_skp_global = ? AND jenis_kinerja = 'kinerja utama' ORDER BY id_skp_global";
    ...
    $sql_tambahan = "SELECT * FROM skp_kuantitatif_awal_tahun_pegawai WHERE id_skp_global = ? AND jenis_kinerja = 'kinerja tambahan' ORDER BY id_skp_global";
    ...
    $sql_perilaku = "SELECT * FROM skp_perilaku_awal_tahun_pegawai WHERE id_skp_global = ? LIMIT 1";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif (isset($_SESSION['last_submit_time']) && (time() - $_SESSION['last_submit_time']) < 5) {
        ...
        // Generate base id_skp_global (for skp_kuantitatif_awal_tahun_pegawai / triwulan tables)
        $base_id_skp_global = 1;
        if ($edit_mode && isset($_POST['save_edit'])) {
            $base_id_skp_global = $edit_id_skp_global;
        } else {
            $max_global_sql = "SELECT MAX(CAST(id_skp_global AS UNSIGNED)) as max_global FROM skp_kuantitatif_awal_tahun_pegawai FOR UPDATE";
        ...
        if (empty($errorMsg)) {
            if ($edit_mode) {
                ...
                $get_triwulan_sql = "SELECT DISTINCT TRIWULAN FROM skp_kuantitatif_awal_tahun_pegawai WHERE id_skp_global = ? LIMIT 1";
            ...
            foreach ($triwulan_list as $triwulan) {
                $check_sql = "SELECT COUNT(*) as count FROM skp_kuantitatif_awal_tahun_pegawai WHERE NIP = ? AND TRIWULAN = ? AND TAHUN = ?";
                if ($edit_mode) {
        ...
        if (empty($errorMsg)) {
            $utama_pimpinan = $_POST['utama_pimpinan'] ?? [];
            $utama_kerja = $_POST['utama_kerja'] ?? [];
            $utama_aspek = $_POST['utama_aspek'] ?? [];
            $utama_indikator = $_POST['utama_indikator'] ?? [];
            $utama_target = $_POST['utama_target'] ?? [];
            $count = max(count($utama_pimpinan), count($utama_kerja), count($utama_aspek), count($utama_indikator), count($utama_target));
            $foundFilledRow = false;
            for ($i = 0; $i < $count; $i++) {
                $r1 = trim($utama_pimpinan[$i] ?? '');
                $r2 = trim($utama_kerja[$i] ?? '');
                $r3 = trim($utama_aspek[$i] ?? '');
        ...
        foreach ($triwulan_list as $triwulan) {
            ...
            if (empty($errorMsg) && isset($_POST['utama_pimpinan']) && !empty(array_filter($_POST['utama_pimpinan']))) {
                // Delete all existing Kinerja Utama for this SKP
                $delete_utama = $conn->prepare("DELETE FROM skp_kuantitatif_awal_tahun_pegawai WHERE id_skp_global = ? AND jenis_kinerja = 'kinerja utama'");
                ...
                $count = count($_POST['utama_pimpinan']);
                for ($i = 0; $i < $count; $i++) {
                    $rhk = $_POST['utama_pimpinan'][$i] ?? '';
            ...
            if (isset($_POST['tambahan_pimpinan']) && !empty(array_filter($_POST['tambahan_pimpinan']))) {
                // Delete all existing Kinerja Tambahan for this SKP
                $delete_tambahan = $conn->prepare("DELETE FROM skp_kuantitatif_awal_tahun_pegawai WHERE id_skp_global = ? AND jenis_kinerja = 'kinerja tambahan'");
                ...
                $count = count($_POST['tambahan_pimpinan']);
                for ($i = 0; $i < $count; $i++) {
                    $rhk = $_POST['tambahan_pimpinan'][$i] ?? '';
                    ...
                    if (!empty($rhk) || !empty($rencana) || !empty($aspek) || !empty($indikator) || !empty($target) || !empty($satuan)) {
                        $stmt = $conn->prepare("INSERT INTO skp_kuantitatif_awal_tahun_pegawai (NAMA, NIP, NAMA_ATASAN_LANGSUNG, NIP_ATASAN_LANGSUNG, PANGKAT_GOL_RUANG, JABATAN, UNIT_KERJA, SATUAN_KERJA, RHK_PIMPINAN_INTERV, RENCANA_HASIL_KERJA, ASPEK, INDIKATOR_KINERJA_INDIVIDU, TARGET, REALISASI_BERDASARKAN_BUKTI_DUKUNG, SATUAN, TRIWULAN, TAHUN, STATUS, TANGGAL_INPUT_SKP, ID_SKP_GLOBAL, JENIS_KINERJA) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?, ?)");
                        if ($stmt) {
            ...
            if (empty($errorMsg) && isset($_POST['perilaku_ekspektasi'])) {
                ...
                $delete_perilaku = $conn->prepare("DELETE FROM skp_perilaku_awal_tahun_pegawai WHERE id_skp_global = ?");
        ...
        if (empty($errorMsg) && $all_triwulan_saved) {
            ...
            header('Location: skp_kuantitatif.php?success=save_berhasil');
            exit();
        } else {
            $conn->rollback();
            if (empty($errorMsg)) {
...
<head>
    ...
    <title><?= $edit_mode ? 'Edit SKP Kuantitatif' : 'SKP Kuantitatif Baru' ?> - SI-APA</title>
    ...
    <style>
        * {
        ...
        .skp-grid-header {
            ...
            border-radius: 6px;
            display: grid;
            grid-template-columns: 40px 1.5fr 1.5fr 0.9fr 1.2fr 0.9fr 0.8fr 56px;
            gap: 8px;
            font-weight: 600;
            margin-bottom: 6px;
            font-size: 0.75rem;
        ...
        @media (min-width: 1200px) {
            .skp-grid-header {
                grid-template-columns: 44px 1.5fr 1.5fr 0.9fr 1.2fr 0.9fr 0.8fr 60px;
                gap: 10px;
                padding: 10px 12px;
                font-size: 0.8rem;
            }
        ...
        @media (min-width: 1600px) {
            .skp-grid-header {
                grid-template-columns: 48px 1.5fr 1.5fr 0.9fr 1.2fr 0.9fr 0.8fr 60px;
                gap: 10px;
                padding: 10px 12px;
            }
            .skp-grid {
    ...
    <div class="main-content">
        ...
        <?php if ($edit_mode): ?>
            <div style="background: #fff3cd; color: #856404; padding: 12px 20px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #ffc107;">
                <strong>Mode Edit:</strong> Anda sedang mengedit SKP Kuantitatif Tahun <?= htmlspecialchars($edit_skp_data['TAHUN'] ?? '') ?>
            </div>
        <?php endif; ?>
        ...
        <form id="skp-form" method="post" <?= !empty($pegawai_error) ? 'style="opacity: 0.6; pointer-events: none;"' : '' ?>>
            ...
            <div class="skp-grid-header">
                <div>NO</div>
                <div>RENCANA HASIL KERJA PIMPINAN YANG DIINTERVENSI</div>
            ...
            <div class="skp-grid" id="utama-table">
                <?php if ($edit_mode && !empty($edit_utama_data)): ?>
                    <?php foreach ($edit_utama_data as $index => $item): ?>
                        <div class="skp-item">
                            ...
                            <div class="skp-cell" data-label="Rencana Hasil Kerja Pimpinan"><textarea class="skp-textarea" name="utama_pimpinan[]" required><?= htmlspecialchars($item['RHK_PIMPINAN_INTERV'] ?? '') ?></textarea></div>
                            ...
                            <div class="skp-cell" data-label="Aspek">
                                <select name="utama_aspek[]" class="dropdown" required>
                ...
                <?php else: ?>
                    <div class="skp-item">
                        ...
                        <div class="skp-cell" data-label="Rencana Hasil Kerja Pimpinan"><textarea class="skp-textarea" name="utama_pimpinan[]" required></textarea></div>
            ...
            <div class="skp-grid" id="tambahan-table">
                <?php if ($edit_mode && !empty($edit_tambahan_data)): ?>
                    <?php foreach ($edit_tambahan_data as $index => $item): ?>
                        <div class="skp-item">
                            <div class="skp-cell" data-label="No"><?= $index + 1 ?></div>
                            <div class="skp-cell" data-label="Rencana Hasil Kerja Pimpinan"><textarea class="skp-textarea" name="tambahan_pimpinan[]"><?= htmlspecialchars($item['RHK_PIMPINAN_INTERV'] ?? '') ?></textarea></div>
                            ...
                            <div class="skp-cell" data-label="Aspek">
                                <select name="tambahan_aspek[]" class="dropdown">
                                    <option value="kuantitas" <?= ($item['ASPEK'] ?? '') === 'kuantitas' ? 'selected' : '' ?>>kuantitas</option>
                                    <option value="kualitas" <?= ($item['ASPEK'] ?? '') === 'kualitas' ? 'selected' : '' ?>>kualitas</option>
                                    <option value="waktu" <?= ($item['ASPEK'] ?? '') === 'waktu' ? 'selected' : '' ?>>waktu</option>
                                </select>
                ...
                <?php else: ?>
                    <div class="skp-item">
                        <div class="skp-cell" data-label="No">1</div>
                        <div class="skp-cell" data-label="Rencana Hasil Kerja Pimpinan"><textarea class="skp-textarea" name="tambahan_pimpinan[]"></textarea></div>
...
- Menggerakan pemanfaatan berbagai sumber daya untuk tujuan bersama.</div>
    ...
    function addRow(tableId) {
        ...
        wrapper.innerHTML =
            ...
            '<div class="skp-cell" data-label="Rencana Hasil Kerja Pimpinan"><textarea class="skp-textarea" ' + (isUtama ? 'required ' : '') + 'name="' + (isUtama ? 'utama_pimpinan[]' : 'tambahan_pimpinan[]') + '"></textarea></div>' +
            ...
            '<div class="skp-cell" data-label="Aspek"><select class="dropdown" name="' + (isUtama ? 'utama_aspek[]' : 'tambahan_aspek[]') + '" ' + (isUtama ? 'required' : '') + '><option value="kuantitas">kuantitas</option><option value="kualitas">kualitas</option><option value="waktu">waktu</option></select></div>' +
