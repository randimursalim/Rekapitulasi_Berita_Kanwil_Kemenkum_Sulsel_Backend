...
try {
    ...
    $skp_check_sql = "SELECT TARGET, REALISASI_BERDASARKAN_BUKTI_DUKUNG, UMPAN_BALIK_DENGAN_BUKTI_DUKUNG, UMPAN_BALIK_STICKER 
                      FROM skp_kuantitatif_awal_tahun_pegawai 
                      WHERE id_skp_global = ?";
    ...
    $perilaku_check_sql = "SELECT " . implode(', ', $perilaku_fields) . " FROM skp_perilaku_awal_tahun_pegawai WHERE id_skp_global = ?";
    ...
    // Update status in skp_kuantitatif_awal_tahun_pegawai table to 'SELESAI EVALUASI' and set evaluation date

    $stmt1 = $conn->prepare("UPDATE skp_kuantitatif_awal_tahun_pegawai SET STATUS = 'SELESAI EVALUASI', TANGGAL_EVALUASI_SKP = NOW() + INTERVAL 8 HOUR WHERE id_skp_global = ?");
    ...
    // Update status in skp_perilaku_awal_tahun_pegawai table to 'SELESAI EVALUASI'
    $stmt2 = $conn->prepare("UPDATE skp_perilaku_awal_tahun_pegawai SET STATUS = 'SELESAI EVALUASI' WHERE id_skp_global = ?");
    if (!$stmt2) {
