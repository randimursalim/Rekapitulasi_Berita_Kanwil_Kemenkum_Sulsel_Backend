...
try {
    ...
    // Update status in skp_kuantitatif_awal_tahun_pegawai table to 'PROSES EVALUASI' (only for SUBMITTED)
    $stmt1 = $conn->prepare("UPDATE skp_kuantitatif_awal_tahun_pegawai SET STATUS = 'PROSES EVALUASI' WHERE id_skp_global = ? AND NIP = ? AND STATUS = 'SUBMITTED'");
    if (!$stmt1) {
    ...
    // Update status in skp_perilaku_awal_tahun_pegawai table to 'PROSES EVALUASI' (only for SUBMITTED)
    $stmt2 = $conn->prepare("UPDATE skp_perilaku_awal_tahun_pegawai SET STATUS = 'PROSES EVALUASI' WHERE id_skp_global = ? AND NIP = ? AND STATUS = 'SUBMITTED'");
    if (!$stmt2) {
