    ...
    elseif (isset($_SESSION['last_submit_time']) && (time() - $_SESSION['last_submit_time']) < 5) {
        ...
        if (empty($errorMsg)) {
            ...
            for ($i = 0; $i < $count; $i++) {
                ...
                $r3 = trim($utama_indikator[$i] ?? '');
                $r4 = trim($utama_target[$i] ?? '');
                $r5 = trim($utama_satuan[$i] ?? '');
                $any = ($r1 !== '' || $r2 !== '' || $r3 !== '' || $r4 !== '' || $r5 !== '');
                $all = ($r1 !== '' && $r2 !== '' && $r3 !== '' && $r4 !== '' && $r5 !== '');
                if ($any && !$all) {
        ...
        foreach ($triwulan_list as $triwulan) {
            ...
            if (empty($errorMsg) && isset($_POST['utama_kerja']) && !empty(array_filter($_POST['utama_kerja']))) {
                ...
                for ($i = 0; $i < $count; $i++) {
                    ...
                    if ($rencana !== '' && $indikator !== '' && $target !== '' && $satuan !== '' && $perspektif !== '') {
            ...
            if (isset($_POST['tambahan_kerja']) && !empty(array_filter($_POST['tambahan_kerja']))) {
                ...
                $count = count($_POST['tambahan_kerja']);
                for ($i = 0; $i < $count; $i++) {
                    $rhk = '';
                    ...
                    if (!empty($perspektif) || !empty($rencana) || !empty($indikator) || !empty($target) || !empty($satuan)) {
                        $stmt = $conn->prepare("INSERT INTO skp_kuantitatif_awal_tahun_pegawai (NAMA, NIP, NAMA_ATASAN_LANGSUNG, NIP_ATASAN_LANGSUNG, PANGKAT_GOL_RUANG, JABATAN, UNIT_KERJA, SATUAN_KERJA, RHK_PIMPINAN_INTERV, RENCANA_HASIL_KERJA, ASPEK, INDIKATOR_KINERJA_INDIVIDU, TARGET, REALISASI_BERDASARKAN_BUKTI_DUKUNG, SATUAN, TRIWULAN, TAHUN, STATUS, TANGGAL_INPUT_SKP, ID_SKP_GLOBAL, JENIS_KINERJA, PERSPEKTIF) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW() + INTERVAL 8 HOUR, ?, ?, ?)");
                        if ($stmt) {
    ...
    <style>
        * {
        ...
        .skp-grid-header {
            ...
            border-radius: 6px;
            display: grid;
            grid-template-columns: 40px 1.2fr 1fr 0.7fr 0.6fr 0.8fr 56px;
            gap: 8px;
            font-weight: 600;
            margin-bottom: 6px;
            font-size: 0.75rem;
        ...
        @media (min-width: 1200px) {
            .skp-grid-header {
                grid-template-columns: 44px 1.2fr 1fr 0.7fr 0.6fr 0.8fr 60px;
                gap: 10px;
                padding: 10px 12px;
                font-size: 0.8rem;
            }
        ...
        @media (min-width: 1600px) {
            .skp-grid-header {
                grid-template-columns: 48px 1.2fr 1fr 0.7fr 0.6fr 0.8fr 60px;
                gap: 10px;
                padding: 10px 12px;
            }
            .skp-grid {
        ...
        @media (max-width: 1024px) and (min-width: 769px) {
            .skp-grid-header {
                grid-template-columns: 44px 1.5fr 1.2fr 0.9fr 0.8fr 60px;
                gap: 8px;
                padding: 10px 12px;
                font-size: 0.8rem;
            }
