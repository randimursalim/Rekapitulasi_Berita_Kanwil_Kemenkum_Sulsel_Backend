...
// Fetch SKP data - Annual SKP only (from skp_kuantitatif_awal_tahun_pegawai)
$is_annual = true;
$skp_data = [];

// Query annual SKP table only
$annual_sql = "SELECT * FROM skp_kuantitatif_awal_tahun_pegawai WHERE id_skp_global = ? ORDER BY ID_SKP ASC";
...
if ($annual_result && $annual_result->num_rows > 0) {
    ...
    die('Data SKP Tahunan tidak ditemukan');
}
$annual_stmt->close();

// Fetch Pegawai (dinilai) detail from historical data
$pegawai_detail = null;
if (!empty($skp_data[0]['NIP']) && !empty($skp_data[0]['TAHUN'])) {
    $nip_pegawai = $skp_data[0]['NIP'];
    $tahun = $skp_data[0]['TAHUN'];
    $q1_sql = "SELECT NAMA, NIP, PANGKAT_GOL_RUANG, JABATAN, UNIT_KERJA, SATUAN_KERJA FROM skp_kuantitatif_awal_tahun_pegawai WHERE NIP = ? AND TAHUN = ? LIMIT 1";
    $stmt_q1 = $conn->prepare($q1_sql);
    $stmt_q1->bind_param('si', $nip_pegawai, $tahun);
    $stmt_q1->execute();
    $res_q1 = $stmt_q1->get_result();
    if ($res_q1 && $res_q1->num_rows > 0) {
        $pegawai_detail = $res_q1->fetch_assoc();
    }
    $stmt_q1->close();
}
...
// Fetch Perilaku Kerja data - Annual SKP only
...
$skp_title = 'Sasaran_Kinerja_Pegawai_Awal_Tahun_Eselon';
$employee_name = $first_row['NAMA'] ?? 'Unknown';
$filename = sprintf('SKP %s_%s_%s.pdf', 
    $skp_title, 
...
<head>
    <meta charset="UTF-8">
    <title>SKP Awal Tahun Eselon - <?= htmlspecialchars($employee_name) ?> - <?= htmlspecialchars($tahun) ?></title>
...
<body>
     <div class="header-title">SASARAN KINERJA PEGAWAI</div>
     ...
     <div class="header-title">TAHUN <?= $tahun ?></div>
     <br>
     ...
     <tbody>
         <tr><th colspan="5" class="blue-bg" style="text-align:left;">HASIL KERJA</th></tr>
         <tr>
             <th class="blue-bg center" style="width: 5%;">NO</th>
             <th class="blue-bg center" style="width: 35%;">RENCANA HASIL KERJA</th>
             <th class="blue-bg center" style="width: 25%;">INDIKATOR KINERJA INDIVIDU</th>
             <th class="blue-bg center" style="width: 20%;">PERSPEKTIF</th>
             <th class="blue-bg center" style="width: 15%;">TARGET</th>
         </tr>
         <tr>
         ...
         <tr><td colspan="5" class="section-row">A. KINERJA UTAMA</td></tr>
         ...
         <?php endif; ?>
         <tr><td colspan="5" class="section-row">B. KINERJA TAMBAHAN</td></tr>
         ...
         <tbody>
             ...
             $perilaku_desc = [
                 ...
                 [
                     ...
                     'Menggunakan kekayaan dan barang milik negara secara bertanggungjawab, efektif and efisien',
