...
$skp_sql = "SELECT * FROM skp_akhir_pegawai WHERE ID_SKP_GLOBAL = ? ORDER BY TANGGAL_INPUT_SKP ASC";
...
$periode_penilaian = "PERIODE PENILAIAN: 01 JANUARI SD 31 DESEMBER TAHUN $tahun";
...
$skp_title = 'Evaluasi_Kuantitatif_Akhir';
...
<head>
    <meta charset="UTF-8">
    <title>SKP Evaluasi Kuantitatif Akhir - <?= htmlspecialchars($employee_name) ?> - <?= htmlspecialchars($period) ?> - <?= htmlspecialchars($tahun) ?></title>
    ...
    <style>
        ...
        @media print {
            ...
            td[rowspan], th[rowspan] {
                /* Default: allow breaks for rowspan cells */
                /* But ensure they break at row boundaries, not in the middle */
                page-break-inside: auto !important;
                page-break-before: auto !important;
                page-break-after: auto !important;
                /* Ensure rowspan cells can break but continue properly */
                /* The browser should handle continuation, but we'll help it */
                break-inside: auto;
            }
            ...
            /* When rowspan breaks, ensure continuation doesn't create extra cells */
            tbody tr {
                break-inside: auto;
            }
            ...
            /* Prevent extra unmerged rows when rowspan breaks across pages */
            /* When a rowspan cell breaks, browsers may create a continuation row on the new page */
            /* This row appears unmerged because the rowspan cell is still on the previous page */
            /* Solution: Use CSS to prevent continuation rows from being displayed */
            tbody tr {
                page-break-before: auto;
            }
            /* Ensure rowspan cells break properly */
            td[rowspan] {
            ...
            /* Prevent continuation rows by ensuring table structure is maintained */
            /* When rowspan breaks, the continuation row will have cells but missing rowspan columns */
            /* The continuation row appears because the rowspan cell is on the previous page */
            /* Solution: Use CSS to hide rows that are likely continuation rows */
            /* Continuation rows typically have the same data cells but are missing the rowspan cells */
            /* We can't perfectly detect this with CSS, but we can use a workaround: */
            /* Ensure that when a rowspan breaks, it doesn't create a visible continuation row */
            table {
                table-layout: auto;
            }
            /* Additional safeguard: ensure rows don't create orphaned continuation structures */
            tbody tr:empty {
                display: none !important;
            }
            /* Force rowspan cells to break cleanly */
            /* When a large rowspan breaks, ensure the continuation is handled properly */
            /* Use a combination of break properties to prevent duplicate rows */
            /* The key is to ensure rowspan cells can break but don't create continuation rows */
            /* This is a browser limitation - we'll minimize it with the above rules */
            /* For the continuation row issue, we need to ensure the rowspan continues properly */
            /* Try using page-break-before on rows to prevent continuation row creation */
            tbody tr:first-child {
                page-break-before: auto;
            }
            /* Prevent continuation rows when rowspan breaks across pages */
            /* Rows that are part of a rowspan group (continuation rows) are marked with class */
            /* Hide continuation rows that appear when rowspan breaks across pages */
            .rowspan-continuation {
                /* These rows are part of a rowspan group but don't have the rowspan cells */
                /* When rowspan breaks across pages, browser may create duplicate continuation rows */
                /* We'll handle this by ensuring proper page breaks */
                page-break-inside: auto;
            }
            /* Additional safeguard: ensure table structure prevents duplicate rows */
            table tbody {
                display: table-row-group;
            }
            /* When a rowspan breaks, the browser should continue it properly */
            /* But if it creates an extra continuation row, we need to handle it */
            /* The rowspan-continuation class helps identify these rows */
            /* However, browser-created continuation rows won't have this class */
            /* So we'll rely on proper page-break properties to minimize the issue */
            ...
            /* Fix: Prevent rowspan cells from appearing unmerged when breaking across pages */
            /* When a rowspan breaks, the browser should continue it on the new page */
            /* However, sometimes it creates an unmerged cell instead of continuing */
            /* Solution: Use CSS to help the browser properly continue rowspan */
            
            /* The issue: When rowspan breaks, browser creates continuation cell on new page */
            /* This cell should be part of the rowspan but appears unmerged */
            /* We can't fully fix this with CSS, but we can minimize it */
            
            /* Ensure rowspan cells break at proper boundaries */
            /* The browser should automatically continue rowspan, but we'll help it */
            /* Key: Allow breaks but ensure they happen at row boundaries */
            
            /* Additional fix: Use display properties to help browser handle rowspan continuation */
            /* Ensure table structure is maintained when rowspan breaks */
            table {
                border-collapse: separate !important;
                border-spacing: 0 !important;
            }
            
            /* When a rowspan breaks, ensure the continuation row is handled properly */
            /* The browser's table engine should continue the rowspan automatically */
            /* But sometimes it creates an unmerged cell - this is a browser limitation */
            
            /* Workaround: Try to prevent the rowspan from breaking in a way that creates unmerged cells */
            /* We'll use a combination of break properties */
            tbody tr:first-child {
                /* First row on new page should continue rowspan properly */
                page-break-before: auto;
            }
            
            /* Note: The unmerged cell issue is a known browser limitation with rowspan and page breaks */
            /* CSS cannot fully fix this, but we've set properties to minimize it */
            /* The browser's table rendering engine handles rowspan continuation */
            /* If unmerged cells still appear, consider using a PDF library that handles rowspan better */
            .no-bottom-border { border-bottom: none !important; }
            table tr:last-child td, table tr:last-child th { border-bottom: none !important; }
        }
    </style>
...
<body>
    ...
    <div class="header-title">PERIODE: AKHIR</div>
    <br>
    <div style="display: flex; justify-content: space-between; align-items: center; margin: 10px 0;">
        <div style="width: 50%;">KEMENTERIAN HUKUM</div>
        <div style="width: 50%; text-align: right;"><?= htmlspecialchars($periode_penilaian ?? '') ?></div>
    </div>
    <table>
        <tr>
    ...
    <div style="display: flex; align-items: center; margin-bottom: 20px; background: #b7d6f6; padding: 10px; border: 1px solid #003366;">
        ...
        $possible_paths = [
            ...
            $_SERVER['DOCUMENT_ROOT'] . '/siapakemenkumsulsel.id/images/' . $image_filename,
            $_SERVER['DOCUMENT_ROOT'] . '/siapakemenkumsulsel.id/images/' . strtolower($image_filename)
        ];
    ...
    <tbody>
        ...
        <?php if (!empty($kinerja_utama)): ?>
            ...
            ?>
            <tr class="<?= !$should_show_rhk && !$should_show_rencana ? 'rowspan-continuation' : '' ?>">
                ...
                <td class="feedback-cell">
                    <?php if (!empty($row['UMPAN_BALIK_STICKER']) && $row['UMPAN_BALIK_STICKER'] == 'C'): ?>
                        <span style="font-family: 'Wingdings', 'Wingdings 2', 'Wingdings 3', 'Arial', sans-serif; font-size: 18px; color:rgb(8, 9, 8); margin-right: 8px;">C</span>
                    <?php endif; ?>
        ...
        <?php if (!empty($kinerja_tambahan)): ?>
            ...
            <tr class="<?= !$should_show_rhk && !$should_show_rencana ? 'rowspan-continuation' : '' ?>">
                ...
                <td class="feedback-cell">
                    <?php if (!empty($row['UMPAN_BALIK_STICKER']) && $row['UMPAN_BALIK_STICKER'] == 'C'): ?>
                        <span style="font-family: 'Wingdings', 'Wingdings 2', 'Wingdings 3', 'Arial', sans-serif; font-size: 18px; color:rgb(0, 0, 0); margin-right: 8px;">C</span>
                    <?php endif; ?>
                    <?= nl2br(htmlspecialchars($row['UMPAN_BALIK_DENGAN_BUKTI_DUKUNG'] ?? '')) ?>
                </td>
            </tr>
    ...
    <table style="width:100%; margin-top:0;">
        ...
        <tr>
            <td colspan="4" class="section-row no-top-border" style="font-weight:normal;"><?= htmlspecialchars($first_row['RATING_HASIL_KERJA'] ?? '') ?></td>
                </tr>
    </table>
    <br>
