    ...
    if (isset($_POST['utama_id'])) {
        foreach ($_POST['utama_id'] as $i => $id) {
            if ($is_skp_akhir) {
                ...
                // For regular SKP, use skp_kuantitatif_awal_tahun_pegawai table
                $stmt = $conn->prepare("UPDATE skp_kuantitatif_awal_tahun_pegawai SET RHK_PIMPINAN_INTERV=?, RENCANA_HASIL_KERJA=?, ASPEK=?, INDIKATOR_KINERJA_INDIVIDU=?, TARGET=?, REALISASI_BERDASARKAN_BUKTI_DUKUNG=?, SATUAN=? WHERE id=? AND NIP=?");
            }
