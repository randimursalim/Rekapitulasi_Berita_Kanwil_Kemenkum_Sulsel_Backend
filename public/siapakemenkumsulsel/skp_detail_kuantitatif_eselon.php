...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['evaluate_skp'])) {
        ...
        foreach ($skp_feedback as $index => $feedback) {
            try {
                ...
                if ($is_skp_akhir) {
                    ...
                    $stmt_check_activity = $conn->prepare("SELECT TARGET, REALISASI_BERDASARKAN_BUKTI_DUKUNG FROM skp_kuantitatif_awal_tahun_pegawai WHERE id_skp_global = ? ORDER BY id_skp LIMIT 1 OFFSET ?");
                }
                ...
                if (!$nip) {
                    ...
                    $stmt_check = $conn->prepare("SELECT NIP FROM skp_kuantitatif_awal_tahun_pegawai WHERE id_skp_global = ? LIMIT 1 OFFSET ?");
                    if ($stmt_check) {
                        $stmt_check->bind_param('ii', $id_skp_global, $index);
                        $stmt_check->execute();
                        $result_check = $stmt_check->get_result();
                ...
                if ($nip) {
                    $stmt = $conn->prepare("UPDATE skp_kuantitatif_awal_tahun_pegawai SET UMPAN_BALIK_DENGAN_BUKTI_DUKUNG = ?, UMPAN_BALIK_STICKER = ?, TANGGAL_EVALUASI_SKP = NOW() WHERE id_skp_global = ? AND NIP = ?");
                    if ($stmt) {
    ...
    if (isset($_POST['perilaku_feedback']) && is_array($_POST['perilaku_feedback'])) {
        ...
        if (!empty($update_fields)) {
            ...
            if ($is_skp_akhir) {
                ...
                // For regular SKP, use skp_perilaku_awal_tahun_pegawai table
                $sql = "UPDATE skp_perilaku_awal_tahun_pegawai SET " . implode(', ', $update_fields) . " WHERE id_skp_global = ?";
            }
    ...
    if (isset($_POST['rating_perilaku_kerja']) || isset($_POST['predikat_kinerja_pegawai']) || isset($_POST['rating_hasil_kerja']) || isset($_POST['capaian_kinerja_organisasi']) || isset($_POST['umpan_balik_dengan_bukti_dukung']) || isset($_POST['umpan_balik_sticker'])) {
        ...
        if (!empty($rating_fields)) {
            ...
            if ($is_skp_akhir) {
                ...
                $sql = "UPDATE skp_kuantitatif_awal_tahun_pegawai SET " . implode(', ', $rating_fields) . ", TANGGAL_EVALUASI_SKP = NOW() WHERE id_skp_global = ?";
            }
    ...
    if ($update_success) {
        ...
        if ($is_skp_akhir) {
            ...
            header("Location: skp_kuantitatif.php?success=evaluasi_berhasil");
        }
        exit();
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_skp'])) {
    ...
    header('Location: skp_detail_kuantitatif.php?id_skp_global=' . $id_skp_global);
    exit();
}
...
<body>
    <div class="container">
        ...
        <div class="content">
                ...
                <div class="info-grid">
                    ...
                    <div class="info-item">
                        ...
                        <div class="info-value">
                            1 JANUARI SD 31 DESEMBER <?= htmlspecialchars($first_row['TAHUN']) ?>
                        </div>
                    </div>
                    <div class="info-item">
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <?php if ($is_draft): ?>
                                            ...
                                            <input type="hidden" name="utama_realisasi[<?= $index ?>]" value="<?= htmlspecialchars($row['REALISASI_BERDASARKAN_BUKTI_DUKUNG']) ?>">
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <?php if ($is_draft): ?>
                                            ...
                                            <input type="hidden" name="tambahan_realisasi[<?= $index ?>]" value="<?= htmlspecialchars($row['REALISASI_BERDASARKAN_BUKTI_DUKUNG']) ?>">
            ...
            ?>
                <div class="perilaku-item-container">
                    <div class="skp-table-container">
            ...
            <?php if ($is_edit_feedback): ?>
                <div style="text-align: center; margin: 30px 0; padding: 20px; background: #f8f9fa; border-radius: 8px; border: 1px solid #0D2052;">
                    ...
                    <?php else: ?>
                        ...
                        <button type="button" onclick="window.location.href='skp_kuantitatif.php'" style="background: #6c757d; color: white; border: none; padding: 12px 30px; border-radius: 6px; font-size: 16px; font-weight: bold; cursor: pointer; margin-left: 10px;">❌ BATAL</button>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
