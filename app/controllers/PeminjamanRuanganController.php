        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            try {
                ...
                if (empty($data['nama_peminjam']) || empty($data['nama_ruangan']) || 
                    ...
                    header('Location: ' . (defined('BASE_URL') ? BASE_URL : '') . '/index.php?page=tambah-peminjaman-ruangan&status=error');
                    exit;
                }

                // Validasi durasi (minimal 1 jam, maksimal 8 jam)
                if ($data['durasi_kegiatan'] < 1 || $data['durasi_kegiatan'] > 8) {
                    header('Location: ' . (defined('BASE_URL') ? BASE_URL : '') . '/index.php?page=tambah-peminjaman-ruangan&status=error&message=' . urlencode('Durasi kegiatan harus antara 1-8 jam.'));
                    exit;
                }
                ...
                if ($konflik) {
                    ...
                    header('Location: ' . (defined('BASE_URL') ? BASE_URL : '') . '/index.php?page=tambah-peminjaman-ruangan&status=error&message=' . urlencode($pesan_error));
                    exit;
                }

                if ($this->model->tambahPeminjamanRuangan($data)) {
                    header('Location: ' . (defined('BASE_URL') ? BASE_URL : '') . '/index.php?page=tambah-peminjaman-ruangan&status=success');
                } else {
                    header('Location: ' . (defined('BASE_URL') ? BASE_URL : '') . '/index.php?page=tambah-peminjaman-ruangan&status=error');
                }
                exit;
            } catch (Throwable $e) {
                header('Location: ' . (defined('BASE_URL') ? BASE_URL : '') . '/index.php?page=tambah-peminjaman-ruangan&status=error&message=' . urlencode($e->getMessage()));
                exit;
            }
        }
    }
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            try {
                ...
                if (empty($data['nama_peminjam']) || empty($data['nama_ruangan']) || 
                    empty($data['kegiatan']) || empty($data['tanggal_kegiatan']) || 
                    empty($data['waktu_kegiatan'])) {
                    header('Location: ' . (defined('BASE_URL') ? BASE_URL : '') . '/index.php?page=tambah-peminjaman-ruangan-masyarakat&status=error');
                    exit;
                }

                // Validasi durasi (minimal 1 jam, maksimal 8 jam)
                if ($data['durasi_kegiatan'] < 1 || $data['durasi_kegiatan'] > 8) {
                    header('Location: ' . (defined('BASE_URL') ? BASE_URL : '') . '/index.php?page=tambah-peminjaman-ruangan-masyarakat&status=error&message=' . urlencode('Durasi kegiatan harus antara 1-8 jam.'));
                    exit;
                }
                ...
                if ($konflik) {
                    ...
                    header('Location: ' . (defined('BASE_URL') ? BASE_URL : '') . '/index.php?page=tambah-peminjaman-ruangan-masyarakat&status=error&message=' . urlencode($pesan_error));
                    exit;
                }

                if ($this->model->tambahPeminjamanRuangan($data)) {
                    header('Location: ' . (defined('BASE_URL') ? BASE_URL : '') . '/index.php?page=tambah-peminjaman-ruangan-masyarakat&status=success');
                } else {
                    header('Location: ' . (defined('BASE_URL') ? BASE_URL : '') . '/index.php?page=tambah-peminjaman-ruangan-masyarakat&status=error');
                }
                exit;
            } catch (Throwable $e) {
                header('Location: ' . (defined('BASE_URL') ? BASE_URL : '') . '/index.php?page=tambah-peminjaman-ruangan-masyarakat&status=error&message=' . urlencode($e->getMessage()));
                exit;
            }
        }
    }
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            try {
                ...
                if (empty($data['nama_peminjam']) || empty($data['nama_ruangan']) || 
                    empty($data['kegiatan']) || empty($data['tanggal_kegiatan']) || 
                    empty($data['waktu_kegiatan'])) {
                    header('Location: ' . (defined('BASE_URL') ? BASE_URL : '') . '/index.php?page=edit-peminjaman-ruangan&id=' . $id . '&status=error');
                    exit;
                }

                // Validasi durasi (minimal 1 jam, maksimal 8 jam)
                if ($data['durasi_kegiatan'] < 1 || $data['durasi_kegiatan'] > 8) {
                    header('Location: ' . (defined('BASE_URL') ? BASE_URL : '') . '/index.php?page=edit-peminjaman-ruangan&id=' . $id . '&status=error&message=' . urlencode('Durasi kegiatan harus antara 1-8 jam.'));
                    exit;
                }
                ...
                if ($konflik) {
                    ...
                    header('Location: ' . (defined('BASE_URL') ? BASE_URL : '') . '/index.php?page=edit-peminjaman-ruangan&id=' . $id . '&status=error&message=' . urlencode($pesan_error));
                    exit;
                }

                if ($this->model->updatePeminjamanRuangan($id, $data)) {
                    header('Location: ' . (defined('BASE_URL') ? BASE_URL : '') . '/index.php?page=edit-peminjaman-ruangan&id=' . $id . '&status=success');
                } else {
                    header('Location: ' . (defined('BASE_URL') ? BASE_URL : '') . '/index.php?page=edit-peminjaman-ruangan&id=' . $id . '&status=error');
                }
                exit;
            } catch (Throwable $e) {
                header('Location: ' . (defined('BASE_URL') ? BASE_URL : '') . '/index.php?page=edit-peminjaman-ruangan&id=' . $id . '&status=error&message=' . urlencode($e->getMessage()));
                exit;
            }
        }
    }
